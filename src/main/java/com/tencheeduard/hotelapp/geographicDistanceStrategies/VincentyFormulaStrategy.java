...
public class VincentyFormulaStrategy implements GeographicDistanceStrategy {
    ...
    Double SEMI_MAJOR_AXIS_MT = 6378137.0;
    Double SEMI_MINOR_AXIS_MT = 6356752.314245;
    Double FLATTENING = 1 / 298.257223563;
    Double ERROR_TOLERANCE = 1e-12;

    // im not even gonna pretend to understand this, but it's very accurate
    // shamelessly ripped from baeldung.com again
    @Override
    public Distance distance(Point a, Point b) {
        double U1 = Math.atan((1 - FLATTENING) * Math.tan(Math.toRadians(a.latitude)));
        double U2 = Math.atan((1 - FLATTENING) * Math.tan(Math.toRadians(b.latitude)));
        ...
        double longitudeDifference = Math.toRadians(b.longitude - a.longitude);
        ...
        do {
            ...
            longitudeDifference = Math.toRadians(b.longitude - a.longitude) + (1 - C) * FLATTENING * sinAlpha *
        ...
        return new Distance(distanceMt / 1000, Unit.KILOMETERS);
    }
}
